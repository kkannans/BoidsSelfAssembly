...
class Experiment1(HypothesisSpace):
    """
    Experiment 1:(hypothesis space 1)
    - Genome: 3 total (3 weights per boid for separation, alignment, and cohesion)
    """
    # modify args to include view_radius for experiment 1 hyperparameter search
    def __init__(self, initial_positions:np.ndarray, initial_velocities:np.ndarray, view_radius:int):
        """
    ...
    def initialize_boids(self, genome):
        ...
        for i, position in enumerate(self.initial_positions):
            ...
            boid.view_radius = self.view_radius
            boids.append(boid)
        return boids