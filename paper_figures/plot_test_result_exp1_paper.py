...
test_performance_folder = "./test_performance_evolution_e1_afpo_pop50_gen50_new_pwd"
experiment_folder_name = "evolution_e1_afpo_pop50_gen50_new_pwd"
...
def load_random_genome_losses(experiment_folder_name):
    random_test_performance_folder = f"test_performance_random_genome_{experiment_folder_name}"
...
def get_plot_data(model_data, regression_losses, random_genome_losses, test_horizon, model_type):
    ...
    # Compute mean and std across seeds, handling NaN values
    ...
    print(f"Total regression shape after slicing: {total_regression.shape}")
    ...
    print(f"Total random genome shape after slicing: {total_random_genome.shape}")

    # Check for empty or invalid arrays
    if np.all(np.isnan(model_mean)) or np.all(np.isnan(total_regression)) or np.all(np.isnan(total_random_genome)):
        raise ValueError("All values are NaN after slicing. Check END_FRAME and test_horizon values.")
...
def main():
    ...
    # Print the accumulated statistics results
    ...
    if 'accumulation_results' in random_test_statistics:
        ...
        print("\nAccumulated Statistical Test Results (vs Random):")
        print("=====================================")
        for i, t in enumerate(accum['time_points']):
            print(f"Time horizon t={t}:")
            print(f"  p-value: {accum['p_vals'][i]:.6f}")
    ...
    save_path = os.path.join(output_dir, "figure2.png")
